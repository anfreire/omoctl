...
- The main model's variant renders inline as `model (variant: x)` — identical to fallback entries — in every view. Previously the variant was invisible on unchanged entries and variant changes appeared as detached `variant:` property lines below `fallback_models`; they now diff on the model line itself. Within those `-`/`+` lines only the segment that actually changed gets the red/green accent — a variant-only change shows the model dimmed, and vice versa. An explicit `variant: null` vs an absent variant is not reported as a change.
